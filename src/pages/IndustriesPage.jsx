...
const industryDetails = [
  ...
  { name: 'Railway', image: 'https://images.unsplash.com/photo-1474487548417-781cb6d646a3?auto=format&fit=crop&w=900&q=80', desc: 'Railway track and rolling stock fasteners must endure cyclic fatigue, vibration, and weathering over decades of service. We supply track bolts, fish bolts, and coach screws to railway standards.', products: ['Track Bolts & Nuts', 'Fish Bolts', 'Coach Screws', 'Spring Washers', 'Nyloc Lock Nuts'] },
  { name: 'Energy & Power', image: 'https://images.unsplash.com/photo-1473341304170-971dccb5ac1e?auto=format&fit=crop&w=900&q=80', desc: 'Power generation facilities — from thermal plants to solar farms — require fasteners that perform reliably in thermal cycling, outdoor exposure, and high-vibration environments across decades of operation.', products: ['Structural Bolts', 'Solar Panel Mounting Bolts', 'Flange Bolts', 'Hex Nuts', 'Serrated Washers'] },
            ...
            >
              <div className={`grid lg:grid-cols-[2fr_3fr] ${i % 2 === 1 ? 'lg:grid-cols-[3fr_2fr]' : ''}`}>
                ...
                <div className={`relative overflow-hidden ${i % 2 === 1 ? 'lg:order-last' : ''}`}>
                  <img src={ind.image} alt={ind.name} className="h-64 w-full object-cover lg:h-full" />
                  <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, rgba(249,115,22,0.30) 0%, rgba(0,0,0,0.45) 100%)' }} />
                  <div className="absolute bottom-6 left-6">
                    <span className="rounded-xl px-4 py-2 text-sm font-extrabold text-white shadow"
                ...
                <div className="flex flex-col justify-center bg-white p-8 lg:p-10">
                  <p className="text-base leading-7 text-slate-600">{ind.desc}</p>
                  <div className="mt-6">
